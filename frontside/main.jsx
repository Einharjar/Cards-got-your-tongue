...
<div id="lobby">
 ...
 <select id="userID" size="35" onChange={this.onDropdownSelected}>
    <optgroup label="Active Games"></optgroup>
	<option disabled selected value> -- Texas Holdem -- </option>
	   {rows}
</select>
<br/>
<button type="button" onClick={this.joinLobby}>Join Lobby</button>
<button type="button" onClick={this.newLobby}>New Lobby</button>
<button type="button" onClick={this.refreshLobbies}>Refresh</button>
      </form>
	  </div>
	  </div>
...
class Queue extends React.Component {
 ...
 class HoldemComp extends React.Component {
  constructor(props) {
    super(props);
    this.state = {pot: 0, raiseVal: 0, table: [], thisPlayer: [],activePlayer: "", currentPlayer: [],winner: "", test: 0};
	  var newpot = 0;
...
//<button type="button" onClick={this.activateLasers}>Check</button>
//<button type="button" onClick={this.activateLasers}>Bet</button>
//<input type='number'/>
...
<div id="holdemTable">
	...
	<h1>	{uname} </h1>
	<h1>	{bananas}</h1>
<button type="button" disabled={(this.state.activePlayer !== uname) ? 'disabled' : null}  onClick={this.fold} >Fold</button>
<button type="button" disabled={(this.state.activePlayer !== uname) ? 'disabled' : null} onClick={this.call}>Call</button>
<input type="number" disabled={(this.state.activePlayer !== uname) ? 'disabled' : null} value={+this.state.raiseVal} onChange={this.handleChangePot} placeholder="Raise Amount"/>
<button type="button" disabled={(this.state.activePlayer !== uname) ? 'disabled' : null} onClick={this.raise}>Raise</button>
...
const LobbyRow = (props) => {
  return (
    <option value={ props.data.id }>
		{props.data.lobbyname} {props.data.gametype} {props.data.playersCurr} / {props.data.playersMax}
	</option>
  );
}
